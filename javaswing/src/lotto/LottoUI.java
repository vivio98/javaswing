...
public class LottoUI extends JFrame implements ActionListener{
  ...
  public LottoUI() {
	//필드값 초기화를 생성자에서 하는데 
	  ...
	  //처리하고자 하면 메소드를 메소드를 호출하는 문법이 가능하다.
      init();
	...
	btn.addActionListener(this); // JFrame 을 상속받은 객체이므로 가능
	panelNorth.add(btn);
	add(panelNorth,BorderLayout.NORTH);
	add(panelSouth,BorderLayout.SOUTH);
	setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); //우측상단 창닫기 버튼
	setBounds(300,400,1200,300);// 화면에서 x좌표 300 y400 가로 1200 픽셀 세로 300픽셀
	setVisible(true);
}
	@Override
	...
	private Icon getIcon(int i) {
		...
		icon = new ImageIcon(imgPath);
		 return icon;
	//return new ImageIcon(imgPath); 로 처리하면 더나은 코딩
	}
